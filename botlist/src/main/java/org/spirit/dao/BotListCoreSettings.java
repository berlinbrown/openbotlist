...
/**
 * The core settings bean contains general setting variables that can be accessed by any module. For example, the search
 * index directory locations are stored in this bean. Other utilities include session tracking.
 *
 * @author Berlin Brown
 */
public class BotListCoreSettings implements Serializable {
    ...
    /**
     * @param searchIndexDir
     *            the searchIndexDir to set
     */
    ...
    /**
     * @param globalIndexDir
     *            the globalIndexDir to set
     */
    ...
    /**
     * Increase the count of how many users are logged in based on if the user management object is available.
     *
     * @param session
     */
    public void incLoggedInUsers(final HttpSession session) {
        if (session != null) {
            BotListCoreUsersContract coreUser = (BotListCoreUsersContract) session
                    .getAttribute(BotListSessionManager.USER_INFO_OBJECT);
            if (coreUser != null) {
                this.loggedInUsers++;
            }
    ...
    /**
     * Decrease the count of how many users are logged in based on if the user management object is available.
     *
     * @param session
     */
