...
public class BotListUserVisitLogDAOImpl extends HibernateDaoSupport implements BotListUserVisitLogDAO {
        ...
        Integer count = (Integer) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "select count(id) from org.spirit.bean.impl.BotListUserVisitLog log where log.createdOn > :lastWeek");
    ...
    public long getVisitAuditOnDate(final Calendar calendar) {
        ...
        Integer count = (Integer) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "select count(id) from org.spirit.bean.impl.BotListUserVisitLog log where date(log.createdOn) = date(:curDate)");
    ...
    public long getVisitAuditCount() {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select count(id) from org.spirit.bean.impl.BotListUserVisitLog");
        if (list == null)
            return -1;
    ...
    public Calendar getFirstVisit() {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select min(log.createdOn) from org.spirit.bean.impl.BotListUserVisitLog log");
        if (list == null)
            return null;
    ...
    public long getUniqueVisitAuditCount() {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select count(distinct remoteHost) from org.spirit.bean.impl.BotListUserVisitLog");
        if (list == null)
            return -1;
        if (list.get(0) instanceof java.lang.Integer) {
            return ((Integer) list.get(0)).longValue();
