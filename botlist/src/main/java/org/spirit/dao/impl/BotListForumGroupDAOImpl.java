...
public class BotListForumGroupDAOImpl extends HibernateDaoSupport implements BotListForumGroupDAO {
     ...
     */
    public BotListForumGroup readForum(final int id) throws DataAccessException {
        return (BotListForumGroup) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session
                        .createQuery("from org.spirit.bean.impl.BotListForumGroup forum where forum.id = " + id);
