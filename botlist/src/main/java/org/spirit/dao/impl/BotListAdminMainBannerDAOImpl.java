...
public class BotListAdminMainBannerDAOImpl extends HibernateDaoSupport implements BotListAdminMainBannerDAO {
    ...
    public BotListAdminMainBanner readBanner(final String section) throws DataAccessException {
        return (BotListAdminMainBanner) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                // Query query = session.createQuery("from org.spirit.bean.impl.BotListAdminMainBanner banner where
                // banner.section = :section and banner.enabled = 'Y'");
                Query query = session
