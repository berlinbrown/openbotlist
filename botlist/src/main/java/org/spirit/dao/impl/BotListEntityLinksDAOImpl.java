 ...
 * @author Berlin Brown
 */
public class BotListEntityLinksDAOImpl extends HibernateDaoSupport implements BotListEntityLinksDAO {
    ...
    /**
     * When iterating through the entity links, do any preperations on the link before they are displayed to the user.
     */
    ...
    /**
     * List the Entity Links including paging the results, but avoid the lazy initialization of unneeded data and only
     * load user data.
     */
    ...
    public BotListEntityLinks readLinkListing(final int id) throws DataAccessException {
        return (BotListEntityLinks) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session
                        .createQuery("from org.spirit.bean.impl.BotListEntityLinks link where link.id = " + id);
        ...
        return getHibernateTemplate().executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "from org.spirit.bean.impl.BotListEntityLinks links where date(links.createdOn) = date(:curDate)");
    ...
    public List readListingUpToCurrentDate(final Calendar calendar) {
        return getHibernateTemplate().executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "from org.spirit.bean.impl.BotListEntityLinks links where (date(links.createdOn) > date(:curDate)) AND (links.createdOn < NOW())");
    ...
    public long getLinkCommentCount(final int id) {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select count(id) from org.spirit.bean.impl.BotListUserComments where linkId = " + id);
        if (list == null)
            return -1;
    ...
    public long getLinkCommentCountByForum(final int id) {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select count(id) from org.spirit.bean.impl.BotListUserComments where forumId = " + id);
        if (list == null)
            return -1;
        if (list.get(0) instanceof java.lang.Integer) {
            return ((Integer) list.get(0)).longValue();
    ...
    public long getLinkCount() {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select count(id) from org.spirit.bean.impl.BotListEntityLinks");
        if (list == null)
            return -1;
        if (list.get(0) instanceof java.lang.Integer) {
            return ((Integer) list.get(0)).longValue();
