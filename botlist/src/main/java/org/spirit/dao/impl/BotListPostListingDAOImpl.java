...
/**
 */
public class BotListPostListingDAOImpl extends HibernateDaoSupport implements BotListPostListingDAO {
    ...
    public BotListPostListing readPostListing(final int id) throws DataAccessException {
        return (BotListPostListing) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session
                        .createQuery("from org.spirit.bean.impl.BotListPostListing post where post.id = " + id);
