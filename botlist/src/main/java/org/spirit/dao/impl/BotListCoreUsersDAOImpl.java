...
public class BotListCoreUsersDAOImpl extends HibernateDaoSupport implements BotListCoreUsersDAO {
    ...
    public BotListCoreUsers readUser(final String userName) throws DataAccessException {
        return (BotListCoreUsers) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session
                        .createQuery("from org.spirit.bean.impl.BotListCoreUsers user where user.userName = :userName");
        ...
        return (BotListCoreUsers) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "from org.spirit.bean.impl.BotListCoreUsers user where user.userName = :userName order by user.id desc LIMIT 0, 50");
                String finUser = KeywordProcessor.filterAlphaNumeric(userName);
                query.setString("userName", finUser);
    ...
    public BotListCoreUsers readUserId(final Long userId) throws DataAccessException {
        return (BotListCoreUsers) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session
                        .createQuery("from org.spirit.bean.impl.BotListCoreUsers user where user.id = :userId");
    ...
    public BotListCoreUsers readUserAcctVerify(final String userName, final String acctNumber)
        ...
        return (BotListCoreUsers) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "from org.spirit.bean.impl.BotListCoreUsers user where user.userName = :userName and user.accountNumber = :acctNumber");
                String finUser = KeywordProcessor.filterAlphaNumeric(userName);
                query.setString("userName", finUser);
    ...
    public long getCoreUsersCount() {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select count(id) from org.spirit.bean.impl.BotListCoreUsers");
        if (list == null)
            return -1;
