...
public class BotListCityListingDAOImpl extends HibernateDaoSupport implements BotListCityListingDAO {
        ...
        return getHibernateTemplate().executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "from org.spirit.bean.impl.BotListCityListing city where city.cityCategory = :city_type order by city.stateAbbr, city.cityName");
    ...
    public BotListCityListing readCityListing(final int id) throws DataAccessException {
        return (BotListCityListing) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session
    ...
    public long getCityPostCount(final int id) {
        ArrayList list = (ArrayList) getHibernateTemplate()
                .find("select count(id) from org.spirit.bean.impl.BotListPostListing where cityId = " + id);
        if (list == null)
            return -1;
