...
public class BotListUserCommentsDAOImpl extends HibernateDaoSupport implements BotListUserCommentsDAO {
    ...
    public BotListUserComments readComment(final int id) throws DataAccessException {
        ...
        return (BotListUserComments) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery(
                        "from org.spirit.bean.impl.BotListUserComments comment left outer join fetch comment.childComments where comment.id = :commentId");
