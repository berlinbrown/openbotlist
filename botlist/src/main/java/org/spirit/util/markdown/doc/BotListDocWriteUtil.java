 ...
 */
package org.spirit.util.markdown.doc;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

import org.spirit.util.markdown.BotListReplacement;
import org.spirit.util.markdown.BotlistMarkStringUtil;

/**
 * Convert Markdown text into HTML, as per http://daringfireball.net/projects/markdown/ .
 */
public class BotListDocWriteUtil {
    ...
    private BotlistMarkStringUtil doBlockQuotes(BotlistMarkStringUtil markup) {
        Pattern p = Pattern.compile("(" + "(" + "^[ \t]*>[ \t]?" + // > at the start of a line
    ...
    private BotlistMarkStringUtil doLists(BotlistMarkStringUtil text) {
        ...
        String wholeList = "(" + "(" + "[ ]{0," + lessThanTab + "}" + "((?:[@]|\\d+[.]))" + // $3 is first list item
                                                                                            // marker
                "[ ]+" + ")" + "(?s:.+?)" + "(" + "\\z" + // End of input is OK
                "|" + "\\n{2,}" + "(?=\\S)" + // If not end of input, then a new para
                "(?![ ]*" + "(?:[@]|\\d+[.])" + "[ ]+" + ")" + // negative lookahead for another list marker
                ")" + ")";
